...
(function (root, factory) {
    if (typeof module === 'object' && module.exports) {
        ...
        // like Node.
        module.exports = factory(require("./heatshrink-wrapper"));
    } else {
...
*/
  ...
  */
  async function RGBAtoString(rgba, options) {
    options = options||{};
    ...
    if (options.compression) {
      bitData = await heatshrink.compress(bitData);
  ...
  */
  async function canvastoString(canvas, options) {
    options = options||{};
    ...
    var str = await RGBAtoString(rgba, options);
    if (options.updateCanvas)
  ...
  */
  async function imagetoString(img, options) {
    options = options||{};
    var canvas = createCanvas(img.width, img.height);
    var ctx = canvas.getContext("2d");
    ctx.drawImage(img,0,0);
    return await canvastoString(canvas, options);
  }
